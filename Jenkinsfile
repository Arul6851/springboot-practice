pipeline {
    ...
    environment {
        ...
        DOCKER_REPO = "backend-app"
        IMAGE_NAME = "backend-app"
        IMAGE_TAG = "${BUILD_NUMBER}"
        KUBE_MANIFEST_REPO = "https://github.com/Arul6851/manifests.git"
    }
    stages {
        stage('Build') {
            steps {
                ...
                withCredentials([string(credentialsId: 'github-token', variable: 'GITHUB_TOKEN')]) {
                    script {
                        sh """
                            ...
                            git clone https://Arul6851:${GITHUB_TOKEN}@github.com/Arul6851/kube-manifests.git kube-manifests
                            cd kube-manifests
                            sed -i 's|image: .*|image: ${DOCKER_REGISTRY}/${DOCKER_REPO}/${IMAGE_NAME}:${IMAGE_TAG}|g' deployment.yaml
                            git config user.email "arulprasad.work@gmail.com"
                            git config user.name "Arul6851"
